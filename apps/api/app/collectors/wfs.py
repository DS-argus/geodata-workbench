...
def _mask_url_secret(value: str) -> str:
    return re.sub(r"(?i)(KEY=)[^&\s)'\"<>]+", r"\1****", value)


def _safe_request_error_message(exc: BaseException) -> str:
    return _mask_url_secret(str(exc))
...
def _fetch_layer_wgs84_bbox(
    *,
    ...
    try:
        ...
        raise ValueError(f"WFS 레이어 범위 요청 실패: {_safe_request_error_message(exc)}") from exc
...
def _fetch_wfs_page(
    *,
    ...
    try:
        ...
        raise ValueError(f"WFS 요청 실패: {_safe_request_error_message(exc)}") from exc

    if response.status_code >= 400:
        detail = _mask_url_secret(_extract_wfs_error_text(response.text))
        ...
        try:
            ...
            raise ValueError(
                f"WFS 요청 실패 (HTTP {response.status_code}): {_safe_request_error_message(exc)}"
            ) from exc
    try:
